...
export default function SpendSlide({ eyebrow, title, standfirst, items, footnote }: Props) {
  ...
  return (
    <Frame eyebrow={eyebrow} title={title} standfirst={standfirst} footnote={footnote}>
      <ol className="m-0 grid list-none gap-x-12 gap-y-0 p-0 border-t border-ink sm:grid-cols-2">
        {items.map((item, n) => (
          <li
            key={item.label}
            className="group border-b border-rule py-[0.8rem] transition-colors duration-200 hover:bg-paper-deep"
          >
              ...
              <p
                className={`font-display m-0 shrink-0 font-light leading-none ${
                  item.value.includes('[')
                    ...
                    : 'text-[1.35rem] text-jelly'
                }`}
              >
                {ink(item.value)}
              </p>
            </div>
            <p className="m-0 mt-1 max-w-[46ch] text-[0.8rem] italic leading-snug text-ink-faint">
              {ink(item.detail)}
            </p>
          </li>
        ))}
      ...
      <p className="mt-5 text-[0.78rem] italic leading-snug text-ink-faint">
